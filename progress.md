...
> Last updated: 2026-05-01 (🏁 **PROJECT SHIPPED** — all 7 phases complete, final whole-tree review APPROVED (0 Blocker / 0 Important), one polish commit (`4ee864d` extract `isUniqueViolation` helper), tagged **v0.1.0**. HEAD: `4ee864d` on `origin/main`).
...
- ⏭️ **Done.** Final whole-tree review APPROVED (0 Blocker / 0 Important / 3 Minor — all on backlog). One reviewer-recommended polish landed (`4ee864d` extracts `isUniqueViolation` from 3 callers to `src/lib/db/errors.ts`). Branch tagged **v0.1.0** at HEAD `4ee864d`; tag pushed to origin. No further action required — backlog items below are forward-looking only.
- 🟡 **Open backlog (forward-looking, none blocking)**: OAuth/`NEXTAUTH_SECRET` still `xxx` placeholders in `.env.local` (only blocks `npm run dev` in a real browser, not tests/CI); Neon DB password should be rotated since it was shared in chat during setup; `as any` casts on `lastContests`/`fetchStatus` in `dashboard/page.tsx` + `as Array<any>`/`(c: any)` in `handles/[platform]/[handle]/page.tsx` (deferred discriminated-union refactor of `getProfile` return shape would resolve all four); `Parameters<typeof authMiddleware>[0]` narrower-cast attempt failed under NextAuth's `(req, event)` 2-arg signature so `middleware.ts` retains an `as unknown as` cast (narrower target type); IDOR test only covers 1/5 member-route surfaces (DELETE favorites — backlog: add 4 mirror cases for groups PATCH/DELETE + favorites PATCH alias-leak + favorites PATCH groupId-leak); e2e happy-path lacks AtCoder leg + filter/delete steps; **CI hardening backlog**: failure-artifact upload (`actions/upload-artifact@v4` on `if: failure()`) + `concurrency:` group + action SHA-pinning + Playwright browser cache + `${{ secrets.* }}` migration once non-E2E CI jobs land.
